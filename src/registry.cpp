  ...
  const string &i_defaultValue)
{
  ...
  if (ERROR_SUCCESS ==
    ...
    if (ERROR_MORE_DATA ==
      ...
      auto buf = static_cast<BYTE *>(_alloca(size));
      if (ERROR_SUCCESS ==
          RegQueryValueEx(hkey, i_name.c_str(), nullptr, &type, buf, &size)) {
        *o_value = (char *)buf;
        RegCloseKey(hkey);
        return true;
      }
  ...
  if (!i_defaultValue.empty()) *o_value = i_defaultValue;
  return false;
}
