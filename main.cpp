...
public:
  ...
  void copy(DWORD i_x, DWORD i_y, const Image *i_image)
  {
    ...
    DWORD width = x2 - i_x;
    for (DWORD y = i_y; y < y2; y ++)
      memcpy(&m_data[y][i_x * 4], &i_image->m_data[y - i_y][0], width * 4);
  }
};
...
Image *readPng(const char *i_filename)
{
  ...
  try
  {
	...
	switch (color_type)
	{
	  ...
	  case PNG_COLOR_TYPE_RGB_ALPHA:
	    *d ++ = (BYTE)((WORD)s[2] * s[3] / 255);
	    *d ++ = (BYTE)((WORD)s[1] * s[3] / 255);
	    *d ++ = (BYTE)((WORD)s[0] * s[3] / 255);
	    *d ++ = s[3];
	    s += 4;
	    break;
	}
  ...
  if (png_ptr)
    png_destroy_read_struct(&png_ptr,
			    info_ptr ? &info_ptr : NULL, (png_infopp)NULL);
...
private:
  void udpate()
  {
    ...
    POINT pt = { 0, 0 };
    UpdateLayeredWindow(m_hwnd, NULL, &m_point, &m_size,
			m_memDc, &pt, RGB(0, 0, 0), &bf, ULW_ALPHA);
  }
  ...
  void draw()
  {
	...
	for (int y = 0; y < imageSize.cy; y ++)
	{
      ...
      {
	StretchDIBits(m_memDc, ptDest.x, ptDest.y, imageSize.cx, imageSize.cy,
		      0, 0, imageSize.cx, imageSize.cy, m_bm, m_bInfo,
	...
	FillRect(m_memDc, &rc, (HBRUSH)GetStockObject(DC_BRUSH));
  ...
  void resize()
  {
    ...
    HBITMAP bitmap =
      CreateDIBSection(m_memDc, (BITMAPINFO *)&bi,
		       DIB_RGB_COLORS, (void **)&m_dib, NULL, 0);
  ...
  bool load(const char *i_filename)
  {
	...
	return false;
      ...
      HLOCAL hBm, hBInfo;
      int error = plugIn->getPicture(i_filename, 0, 0, &hBInfo, &hBm, NULL, 0);
      if (error != SPI::ERROR_NORMAL)
	if (plugIn->getPicture(i_filename, 128, 0, &hBInfo, &hBm, NULL, 0)
	    != SPI::ERROR_NORMAL)
	{
	  clear();
	  char buf[100];
	  ...
	  switch (error)
	  {
	    ...
	    case SPI::ERROR_INTERNAL:
	      m_error += "internal error"; break;
	    case SPI::ERROR_STOP_EXTRACT: m_error += "canceled by user"; break;
	    case SPI::ERROR_UNKNOWN_FORMAT: m_error += "unknown format"; break;
	    case SPI::ERROR_DAMAGED_DATA: m_error += "corrupted picture";break;
	    case SPI::ERROR_OUT_OF_MEMORY: m_error += "out of memory"; break;
	    case SPI::ERROR_READ_FAILED: m_error += "cannot read file"; break;
	  }
	  m_error += " : ";
	  m_error += i_filename;
	  return false;
      ...
      m_bInfo = (BITMAPINFO *)LocalLock(m_hBInfo);
  ...
  void displayCurrent()
  {
	...
	GetTextExtentPoint32(m_memDc, m_error.c_str(), m_error.size(),
			     &m_imageSize);
      if (hasImage())
      {
	if (isError())
	...
	{
	  char buf[1024];
	  snprintf(buf, NUMBER_OF(buf), "%s (%dx%d)", filename,
	...
	MINMAXINFO mmi;
	wmGetMinMaxInfo(&mmi);
	MoveWindow(m_hwnd, m_point.x, m_point.y,
   ...
   bool hasImage()
   {
     return m_image || (m_hBm && m_hBInfo) || isError();
   }
 ...
 private:
       ...
       m_hBInfo(NULL)
   {
	 ...
	 m_displayList.push_back(__argv[i]);
   }
   ...
   // WM_SIZE
   void wmSize(int i_flag, int i_width, int i_height)
   {
     resize();
   }
   ...
   // WM_ACTIVATE
   void wmActivate(int i_flag, HWND i_hwnd)
   {
     draw();
   }
   ...
   {
     size_t n = DragQueryFile(i_hDrop, -1, NULL, 0);
     std::vector<std::string> files;
     files.resize(n);
				  ...
				  WPARAM i_wParam, LPARAM i_lParam)
  {
	...
	case WM_NCLBUTTONDOWN:
	  if (This->wmNcLButtonDown(i_wParam, *(POINT *)&i_lParam))
	    return 0;
	  break;
	case WM_KEYDOWN:
	  if (This->wmKeyDown(i_wParam, i_lParam))
	    return 0;
	  break;
	case WM_DROPFILES:
		   ...
		   LPSTR /* i_lpszCmdLine */, int /* i_nCmdShow */)
{
  ...
  HWND hwndPNGAlphaViewer
    = CreateWindowEx(WS_EX_ACCEPTFILES | WS_EX_LAYERED,
		     "PNGAlphaViewer", NULL, WS_OVERLAPPEDWINDOW,
